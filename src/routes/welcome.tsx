...
export const Route = createFileRoute("/welcome")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A secure introduction to your grant application dashboard and next steps.",
      },
    ],
  }),
...
function WelcomePage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("grantWelcomePending") !== "true") {
    ...
    if (!token) {
      ...
      navigate({ to: "/apply", search: { tab: "signin" }, replace: true });
      return;
    }
    let active = true;
    getDashboardDataServerFn({ data: { token } })
      .then((result: any) => {
        if (!active) return;
        ...
        <div className="mx-auto max-w-5xl">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
              ...
              <div className="flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-4 max-w-xl text-sm leading-6 text-blue-100 sm:text-base">
                    Your account is ready. This quick introduction explains what you can do inside
                    your grant portal before you enter your personal dashboard.
                  </p>
                  {fullName && (
            ...
            <div className="p-6 sm:p-10">
              {loading ? (
                <div className="mb-8 flex items-center justify-center gap-2 rounded-xl border border-blue-100 bg-blue-50 py-4 text-sm font-semibold text-blue-900">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  Loading your secure reference...
                </div>
              ) : (
                <div className="mb-8 rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-5 sm:p-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 break-all font-mono text-xl font-bold tracking-wider text-blue-950 sm:text-2xl">
                        {user.refNumber || "Reference available in your dashboard"}
                      </p>
                      <p className="mt-2 text-xs leading-5 text-emerald-800">
                        Keep this reference private. Do not post it publicly or share it with
                        strangers.
                      </p>
                    </div>
                  </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <InfoCard
                  ...
                  text="Review the information connected to your application and keep your account details up to date."
                />
                <InfoCard
              ...
              <div className="mt-8 flex flex-col items-stretch gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">Open your dashboard to continue.</p>
                </div>
                <button
                  type="button"
...
function InfoCard({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-3 inline-flex rounded-lg bg-blue-950 p-2 text-amber-300">{icon}</div>
