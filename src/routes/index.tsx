...
function Home() {
  return (
    ...
    body: "Complete the online form with your project summary, budget, and contact details. You can save a copy for your records.",
  },
  {
    title: "3. Review",
    body: "A volunteer review panel scores applications against the published rubric. Typical review time is six to eight weeks.",
  },
  {
    title: "4. Decision",
    body: "Every applicant receives a written decision by email, including feedback. Awards are disbursed by bank transfer only.",
  },
];
    ...
    <SiteLayout>
      <section className="border-b border-border bg-card">
        <div className="mx-auto max-w-6xl px-4 py-16">
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
            Independent · Non-governmental · Demonstration project
          </p>
          <h1 className="mt-4 max-w-3xl text-4xl font-bold leading-tight text-foreground sm:text-5xl">
            Community funding for local projects, applied for transparently
          </h1>
          <p className="mt-5 max-w-2xl text-lg text-muted-foreground">
            The Community Grants Portal is a sample application that shows how an honest grant
            programme works: published criteria, a free application, an open review rubric, and a
            written decision for everyone who applies.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link
              to="/apply"
              className="rounded-sm border border-input bg-background px-5 py-3 text-sm font-semibold text-foreground transition-colors hover:bg-secondary"
            >
      ...
      <section className="mx-auto max-w-6xl px-4 py-14">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((s) => (
            <div key={s.title} className="rounded-sm border border-border bg-card p-5">
              <h3 className="font-serif text-base font-bold text-foreground">{s.title}</h3>
      ...
      <section className="border-y border-border bg-secondary">
        <div className="mx-auto max-w-6xl px-4 py-14">
          <h2 className="text-2xl font-bold text-foreground">How to spot a grant scam</h2>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Real funders never behave the way scammers do. Walk away if you see any of these:
          </p>
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {[
              "You are told you already won a grant you never applied for.",
              "You are asked to pay a tax, clearance, insurance, or delivery fee first.",
              "Funds are described as cash delivered to your home by courier.",
              "You are pressured by a countdown or a 24 to 48 hour deadline.",
              "You are told to keep the award secret from family or your bank.",
              "The sender claims to be a government agency from a non-official domain.",
            ].map((warning) => (
              <li
                key={warning}
                className="rounded-sm border border-border bg-card p-4 text-sm text-foreground"
              >
                {warning}
              </li>
            ))}
      ...
      <section className="mx-auto max-w-6xl px-4 py-14">
        <div className="rounded-sm border border-border bg-card p-6">
          ...
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
            This site is a portfolio and coursework project. It does not distribute real money, is
            not connected to any government or funder, and every programme name, statistic, and
            testimonial you might add should be clearly marked as fictional. Use it to demonstrate
            routing, forms, validation, authentication, and data storage.
          </p>
        </div>
      </section>
    </SiteLayout>
